...
- **M3 MCP 标准化翻译**：后端经 `mcp` SDK **真实调用** HPO-MCP-Server 的 `search_hpo_terms`，把白话症状标准化为 `hpo_terms`。因 HPO API 仅索引英文，须先由 M3 将中文口语抽取为英文医学关键词，查得编码后再回填中文名。会话在 `lifespan` 内常驻复用。
