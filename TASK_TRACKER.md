...
- [ ] **0.5** 跑通最小闭环冒烟：`python scripts/rag_builder.py --dry-run` 必须有「丢弃干预章节」与「用药词拦截」两条计数（计数为 0 即 AC10 失败，停下来报告）
...
- [ ] **1.1.3** 每个切片写入 metadata：`{"bucket": "A"|"B"|"C", "origin": "...", "source": "..."}`，四键缺一不可
- [ ] **1.1.4** collection 名固定 `autism_genetics_knowledge`，从 `config.COLLECTION_NAME` 读取，**禁止在脚本里写死字符串字面量**

### 1.2 章节过滤闸门（AC10 核心）

- [ ] **1.2.1** 识别 `raw/*.txt` 内形如 `## Management` / `## Treatment` / `## Surveillance` / `## Therapeutic` / `## Management and Treatment` 的章节标题，丢弃该章节下全部段落
...
- [ ] **1.3.1** 维护禁用药名词典（最小集合：`risperidone / 利培酮 / aripiprazole / 阿立哌唑 / melatonin / 褪黑素 / ABA / 行为干预 / 言语治疗 / 感统训练 / occupational therapy`），新增词条须经人类审核
...
- [ ] **1.5.1** `python scripts/rag_builder.py --dry-run` 输出双计数均 > 0
- [ ] **1.5.2** 真实灌库后 `chroma_db/` 目录非空，`collection.count() >=` 精编切片数 + 原文幸存切片数
- [ ] **1.5.3** 抽检 3 个切片，其 metadata 四键齐全，`bucket` ∈ {A,B,C}
...
- [ ] **2.1.1** `from config import *` 收口所有环境变量；**`main.py` 全文件 grep 不到 `os.getenv`**（I3）
- [ ] **2.1.2** FastAPI `lifespan` 上下文管理器内启动 MCP stdio 子进程，会话常驻复用（PRD M3）
- [ ] **2.1.3** 启动时若 `MINIMAX_API_KEY` 缺失，立即抛 `MISSING_API_KEY` 而非延迟到第一次请求
...
- [ ] **2.2.5** 定义 `class ScreeningResponse(BaseModel)`：八必填字段一个不少 + `confidence_level: Optional[int] = None`
...
- [ ] **2.3.2** 调用 `mcp_translate_symptoms(symptoms)` → 返回 `List[HpoTerm]`，长度 ≥ 1（AC2）
- [ ] **2.3.3** 调 ChromaDB 以 `hpo_terms.name` + 变异名为 query，返回 `retrieved_chunks: List[Chunk]`
- [ ] **2.3.4** 切片为空时短路：`comparisons=[]`、`vus_reassurance="未匹配到相关指南……"`（I8）
- [ ] **2.3.5** 切片非空时，组装 prompt 注入 system：要求 M3 返回**严格 JSON**，禁止任何解释性文本
- [ ] **2.3.6** 调 `https://api.minimax.io/v1/chat/completions`，`model="MiniMax-M3"`, `temperature=0.3`
- [ ] **2.3.7** JSON 解析失败 / 非 200 / 超时 → 抛 HTTPException(502, `MINIMAX_API_ERROR`)，**不**透传原始文本
- [ ] **2.3.8** 后端**硬填充** `disclaimer` 字段，从 `config.official_disclaimer()` 取桶 C 原文，**不经 LLM**（I1）
...
- [ ] **2.4.5** 全部英文关键词均无命中时返回空数组（**不**抛错），由调用方按 AC2 失败处理
...
- [ ] **2.5.4** 停掉 `.env` 里的 `MINIMAX_API_KEY` 重启 → 期望 HTTP 500 `MISSING_API_KEY`
- [ ] **2.5.5** `grep -n "os.getenv" main.py` 零命中
...
- [ ] **3.0.1** 创建 `.streamlit/config.toml`，内容**逐字**等于 UI_BLUEPRINT §1.5：`base="light"`, `primaryColor="#5B8C85"`, `backgroundColor="#FAFAF7"`, `secondaryBackgroundColor="#F4F3EE"`, `textColor="#1F2321"`, `toolbarMode="minimal"`

### 3.1 启动顺序（每行都不能调换）

- [ ] **3.1.1** 第 1 行：`st.set_page_config(page_title="ASD-GenDecoder", layout="wide", initial_sidebar_state="collapsed")`
- [ ] **3.1.2** 第 2 行：`st.markdown(CSS, unsafe_allow_html=True)`，CSS 取自 UI_BLUEPRINT §7 全文
- [ ] **3.1.3** 初始化 `st.session_state`：`stage="idle"`, `payload=None`, `err=None`, `symptoms=""`, `gene_report=""`
- [ ] **3.1.4** 读 `.env` 的 `BACKEND_URL`，缺省 `http://127.0.0.1:8000`
...
- [ ] **3.5.10** **V10** 等宽字体仅出现在 5 处：`hpo_id` / `gene` / 两类 `source` / 引用编号 / `gene_report` 输入框
...
- [ ] **4.1.1** 读 `data/test_cases/*.json`，每个用例含 `request` / `expect_status` / `expect_fields` / `min_hpo_terms` / `expect_buckets` / `forbid_conditions_any` / `forbid_phrases` 字段
- [ ] **4.1.2** 用例数量 = 7，文件名 `tc_01.json` ~ `tc_07.json`；`tc_05` 为阴性对照

### 4.2 不变量断言（I1–I9）

- [ ] **4.2.1** **I1** `disclaimer.strip() == bucket_c_compliance.json` 中 `is_disclaimer==true` 条目的 `text.strip()`
- [ ] **4.2.2** **I2** `next_steps` 全文同时包含「儿童发育行为科」与「医学遗传科」
- [ ] **4.2.3** **I3** `comparisons[].explanation` ∪ `vus_reassurance` ∪ `next_steps` 中禁用词（确诊 / 患有 / 即为 / 需服用 / 建议用药 + 用例专属追加词）**零命中**
- [ ] **4.2.4** **I4** 每项 `hpo_id` 以 `HP:` 开头；`hpo_terms` 长度 ≥ `min_hpo_terms`
- [ ] **4.2.5** **I5** `comparisons[].condition` 不在 `forbid_conditions_any` 列表
- [ ] **4.2.6** **I6** `retrieved_chunks[].bucket` 覆盖 `expect_buckets`（通常 {A,B,C}）
- [ ] **4.2.7** **I3 坑** 扫描是字面子串，含 `disclaimer` 与 `retrieved_chunks` 在白名单内
- [ ] **4.2.8** **I7** 不强制做字符串断言，但写一行 warning 提示人工抽查 `explanation` ↔ `retrieved_chunks` 可回溯性
- [ ] **4.2.9** **I8** 不强制做字符串断言，但若 `comparisons==[]` 则 `vus_reassurance` 必须含「未匹配到」字样
- [ ] **4.2.10** **I9** 若响应含 `confidence_level`，校验 `vus_reassurance` ∪ `next_steps` ∪ `disclaimer` 中不含「概率 / 可能性 / 几率」
...
> 每条都是**硬红线**，违反即视为功能缺陷而非文案偏好。验收脚本会全自动拦截。
...
- [ ] **5.3** 响应切片中不得出现药名（`risperidone / 利培酮` 等）；若有，验收脚本 AC10 与不变量 I3 双闸拦截
...
- [ ] **6.6** 按 `AGENTS.md §6` 分支命名约定出分支：`feat/be-*` / `feat/fe-*` / `feat/rag-*`
...
- [ ] 我对合规章节（PRD §2 / schema §3.2）的全部不变量都**心中有数**
...
| `tc_05` 阴性对照失败 | 检查 I5：后端可能在拼凑罕见病病名；回查 `vus_reassurance` 是否含禁忌病种 |
