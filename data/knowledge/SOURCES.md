...
**`raw/` 下已按下表预置了 9 份同名占位文件**，无需自己新建、也不用担心敲错文件名。每份占位文件里写明了该篇的来源链接、入库归属与格式要求，**下载后整体替换其内容即可**。

占位文件首行含 `RAG_CORPUS_PLACEHOLDER` 标记，灌库脚本识别到该标记会跳过并提示「待填充」，因此说明文字不会被灌进向量库。跑 `python scripts/rag_builder.py --dry-run` 可以随时看还剩几份没填。
...
- **PubMed 与 AAP 摘要要手动补一行 `Abstract` 作为标题**。摘要本身没有章节标题，缺了这行整篇会落入 preamble 被跳过，灌库结果是 0 条切片。
- **不要手动删除 Management 章节**：脚本会自动识别并丢弃，保留原样即可，这样过滤日志能算出准确的丢弃比例。
- 文件名必须与下表**完全一致**，脚本按文件名推断来源与病种。占位文件已经是正确的名字，别改。
